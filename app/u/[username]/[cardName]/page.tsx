...
export async function generateMetadata({
  ...
  params: Promise<{ username: string; cardName: string }>;
}): Promise<Metadata> {
  const { username, cardName } = await params;
  try {
    const profile = await fetchProfile(username);
    const decoded = decodeURIComponent(cardName);
    const card = (profile.cards || []).find(
      (c) => normalizeCardName(c.name) === normalizeCardName(decoded),
    );
    const title = card
      ? `${profile.fullname} — ${card.name}`
...
export default async function PublicCardPage({
  ...
  return (
    <main className="min-h-dvh bg-[var(--page-bg)] px-6" style={style}>
      <div className="mx-auto w-full max-w-md pb-16">
        ...
        <div className="mb-6 text-center text-sm font-semibold text-neutral-800/80">
          {card.name}
        </div>
