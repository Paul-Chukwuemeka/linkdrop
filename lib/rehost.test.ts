...
vi.mock("@/lib/s3", () => ({
  r2: { send: vi.fn() },
  R2_BUCKET: "bucket",
  R2_PUBLIC_URL: "https://cdn.dropcard.example",
}))
...
describe("reHostAvatarUrl", () => {
  ...
  it("returns URLs already on our own storage untouched", async () => {
    const hosted = "https://cdn.dropcard.example/avatars/abc.jpg"
  ...
  it("uploads the image and returns the re-hosted URL", async () => {
    ...
    expect(result).toMatch(/^https:\/\/cdn\.dropcard\.example\/avatars\/[a-f0-9]{64}\.png$/)
